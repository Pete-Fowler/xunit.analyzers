...
public class TimeoutTestMustUseCancellationToken : XunitDiagnosticAnalyzer
{
	public TimeoutTestMustUseCancellationToken()
		: base(Descriptors.X1054_TimeoutTestMustUseCancellationToken) { }
	...
	)
	{
		...
		context.RegisterOperationAction(
			context =>
			{
				...
				if (
					!foundSymbol
					|| lambdaOwner is ILocalFunctionOperation or IAnonymousFunctionOperation
				)
					return;
				...
				if (lambdaOwner is IInvocationOperation lambdaOwnerInvocation)
					if (
						!xunitContainerTypes.Contains(
							lambdaOwnerInvocation.TargetMethod.ContainingType
						)
					)
						return;
				...
				foreach (var parameter in parameters)
					...
					{
						argument = invocationOperation.Arguments.FirstOrDefault(arg =>
							SymbolEqualityComparer.Default.Equals(arg.Parameter, parameter)
						);
						break;
					}
				...
				if (argument is not null)
				{
					...
					else if (argument.Syntax is ArgumentSyntax argumentSyntax)
					{
						...
						if (
							kind
							is SyntaxKind.DefaultExpression
				...
				{
					var targetParameterTypes = parameters
						.Select(p => p.Type)
						.Concat([cancellationTokenType])
						.ToArray();
					foreach (
						var member in invokedMethod.ContainingType.GetMembers(invokedMethod.Name)
					)
							...
							for (var idx = 0; idx < targetParameterTypes.Length; ++idx)
								if (
									!SymbolEqualityComparer.Default.Equals(
										targetParameterTypes[idx],
										methodParameterTypes[idx]
									)
								)
								{
									match = false;
		...
		)
		{
			...
			builder[Constants.Properties.ParameterIndex] = parameter.Ordinal.ToString(
